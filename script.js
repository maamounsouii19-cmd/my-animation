...
let morphTarget = 1; // 1 == heart, 0 == star
let morphProgress = 0;
...
function createUI() {
  ...
  // Disable morphing: keep the heart shape only
  morphBtn.disabled = true;
  morphBtn.title = 'Morph disabled - heart-only mode';
  morphBtn.classList.add('disabled');
...
function createParticleSystem() {
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    // Use heart positions for both the initial positions and the stored heartPosition attribute
    ...
    starPositions[i3] = heartPos.x;
    starPositions[i3 + 1] = heartPos.y;
    starPositions[i3 + 2] = heartPos.z;
